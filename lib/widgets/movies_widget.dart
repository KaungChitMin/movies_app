...
class GenreMovieWidget extends StatelessWidget {
  const GenreMovieWidget({super.key, required this.movieVO});
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: (){
        context.navigateToNextScreen(context,  DetailsPage(movieID: movieVO.id ?? 0));
      },
      child: Container(
          width: kMovieWidth,
          margin: const EdgeInsets.only(right: kSP20x),
...
class MovieNameRatingAndVoteView extends StatelessWidget {
  const MovieNameRatingAndVoteView({super.key, required this.movieName, required this.rating, required this.voteCount});
